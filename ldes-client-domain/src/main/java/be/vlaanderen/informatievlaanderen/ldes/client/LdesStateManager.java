...
public class LdesStateManager {
	...
	private final Long defaultFragmentExpirationInterval;
    ...
    public LdesStateManager(Long defaultFragmentExpirationInterval) {
    	this.defaultFragmentExpirationInterval = defaultFragmentExpirationInterval;
    ...
    public void processedFragment(LdesFragment fragment) {
    	if (fragment.isImmutable()) {
    		...
    		processedMutableFragments.put(fragment.getFragmentId(), Optional.ofNullable(fragment.getExpirationDate())
    				.orElse(LocalDateTime.now().plusSeconds(defaultFragmentExpirationInterval)));
